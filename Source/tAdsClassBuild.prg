...
FUNCTION TAds_CreateClassFromDatabase(f_nConnection,f_cTableName,f_cDirectory)
   ...
   cTxtClass += "               f_lExclusive,;       // Abrir Exclusivamente"+CRLF
   cTxtClass += "               f_lDataLoadOnSkip);  // Executar DataLoad ao movimentar ponteiro"+CRLF
   ...
   cTxtClass += "/*-----------------------------------------------------------------------------*/"+CRLF
   cTxtClass += "METHOD OpenRdd(f_nConexao,f_nCache,f_lExclusive,f_lDataLoadOnSkip) Class "+cClassName+CRLF+CRLF

   cTxtClass += "   Default f_nCache := nCacheAds()"+CRLF+CRLF
  
   cTxtClass += "   ::nCache        := f_nCache"+CRLF
   cTxtClass += "   ::nTpCallRdd    := 2"+CRLF 
   cTxtClass += "   ::NewRdd(::cTableName,f_nConexao,::nCache,f_lExclusive,f_lDataLoadOnSkip)"+CRLF
   ...
   Hb_MemoWrit(cPrgName,cTxtClass)
