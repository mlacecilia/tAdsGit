...
FUNCTION TAds_StructToScript(f_aStructPointer)
   ...
   Elseif f_aStructPointer[2] == "N"  
      If f_aStructPointer[3] == 32  
         cTmp := TAds_Str(f_aStructPointer[3])
      Else
...
FUNCTION TAds_ReindexTable(f_nConnection,f_cTableName,f_nPageSize,f_lErrorDisplay)
   Local oDs_Qry, oDb_Table, lExecute := .F., nAdsVersion := Val(AdsVersion())
   Local lc_aIndexFound := {}

   Default f_nConnection := tAds_GetConnectionDefault(), f_nPageSize := 0 // 0-Default Page size (Será atribuído conforme a necessidade)
   ...
   If Upper(f_cTableName) == "LOG_AUDITORIA"
      Return .T.
   Endif

   lc_aIndexFound := TAds_SystemIndexes(f_nConnection,f_cTableName)

   If Len(lc_aIndexFound) == 0
      Return .T.
   Endif
   ...
   If nAdsVersion >= 10
      ...
      lExecute := oDs_Qry:DsExecute(,f_lErrorDisplay)
      //oDs_Qry:End()
   Else
...
// f_nConnection  > Number Connection in tAdsConnection
FUNCTION TAds_ReindexAllTables(f_nConnection,f_aTablesIgnore,f_lErrorDisplay)
   Local aTablesInDictionary := {}, iFor := 0
   
   Default f_aTablesIgnore := {}, f_lErrorDisplay := .T.
   ...
   For Ifor := 1 To Len(aTablesInDictionary)
      If ascan(f_aTablesIgnore,aTablesInDictionary[iFor]) == 0
         ...
         TAds_ReindexTable(f_nConnection,aTablesInDictionary[iFor],Nil,f_lErrorDisplay)
      EndIf
   Next
...
FUNCTION TAds_CreateTableFromCode(  f_nConnection,;      //Number Connection in tAdsConnection
                                    f_cTableName,;       //Nome da nova tabela
                                    f_aStruct,;          //1-Field Name, 2-Field Type, 3-Field Lengh, 4-Decimal, 5-Index Type, 6-Comment, 7-Default Value
                                    f_cDescription,;     //Descrição da tabela
                                    f_aPropertys)        //1-MemoBlockSize, 2-Triggers enable or disable (.T./.F.), 3-Caching Size, 4-Table Trans free (.F./.T.)
...
RETURN lOkRet
///////////////////////////////////////////////////////////////////////////////
// Criar nova chave de indice em tabela existente 
// f_nConnection > Number Connection in tAdsConnection
// f_cTableName > Nome da nova tabela
// f_cTagIndex > Nome do campo ou novo nome para a chave de indice
// f_cTagExpr > Expressão da chave (nil) campo default ou chave composta ex: NOME+CIDADE
