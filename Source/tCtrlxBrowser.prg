...
Class tCtrlxBrw ///From TXbrowse
   ...
   Data nQtaCharFilter
   Data lGetFilterProcess           As Logic    Init .F. // Se esta executando o filtro em tCtrlxBrw_EditFilterRdd
   Data lGetFilterActivate          As Logic    Init .F. // Se há algum filtro ativado pelos Gets associados
   ...
   Method tCtrlxBrw_Seek(f_oBjCol,f_uVar,f_lSoftSeek,f_lReturnLastOrder)
...
Method SetTAds() Class tCtrlxBrw
   ...
   If nTpGetRelPos == 2 // Open via Sql Dataset cursor vivo 
      ...
      ::oXBrwCtrl:bKeyNo    := <| _n, _Self | 
         Local _nRetPos := 0
         If _n == nil
            _nRetPos := nCalcRelPos((::oDbDs_DataBaseAds:GetRelKeyPos() * _Self:nLen) /10,;
                                    ::oXBrwCtrl,;
                                    ::oDbDs_DataBaseAds) 
         else
            _nRetPos := ::oDbDs_DataBaseAds:SetRelKeyPos( Int(Round((_n+1) / _Self:nLen,3)) ) 
         EndIf
         return _nRetPos
      > //cEnd
   ...
   Else // Via Rdd ou Static Cursor
      ...
      ::oXBrwCtrl:bKeyNo    := <| _n, _Self | 
         Local _nRetPos := 0
         If _n == nil
            _nRetPos := nCalcRelPos((::oDbDs_DataBaseAds:GetRelKeyPos() * _Self:nLen) ,;
                                    ::oXBrwCtrl,;
                                    ::oDbDs_DataBaseAds) 
         else
            _nRetPos := ::oDbDs_DataBaseAds:SetRelKeyPos( Int(Round((_n+1) / _Self:nLen,3)) ) 
...
Method tCtrlxBrw_SetOrder(f_cObjCol) Class tCtrlxBrw
   ...
   If !Empty(::aOrdersCreation[nOrderPos,2])
    ...
    // wndmain():cTiTle := f_cObjCol:cSortOrder
      ...
      f_cObjCol:bClrHeader := {||{nRgb(0,0,0),nRgb(192,192,192),nRgb(192,192,192),nRgb(192,192,192)}}
      ::oXBrwCtrl:Adjust()
      //f_cObjCol:SetOrder() // 2021-05

      //f_cObjCol:bClrHeader := {||{nRgb(0,0,0),nRgb(192,192,192),nRgb(192,192,192),nRgb(192,192,192)}}
      
      //f_cObjCol:PaintHeader( 2, nil, ::oXBrwCtrl:nHeaderHeight - 3 )


   EndIf
   
   //f_cObjCol:RefreshHeaders()
...
Return nSkipper  
/*-----------------------------------------------------------------------------*/
Method tCtrlxBrw_Seek(f_oBjCol,f_uVar,f_lSoftSeek,f_lReturnLastOrder) Class tCtrlxBrw
LOCAL nRecMarca := ::oDbDs_DataBaseAds:Recno(), cTpData := "", lc_cOrderIn

   If Hb_IsNil(f_oBjCol) .or. Hb_IsNil(f_uVar)
      Return Nil
   EndIf
   ...
   If Empty(::oDbDs_DataBaseAds:cOrderFocus)
      ...
      //? "sdcsds"
      ::oDbDs_DataBaseAds:Seek(f_uVar,f_oBjCol:cSortOrder,f_lSoftSeek,f_lReturnLastOrder)
   Else  
      ::oDbDs_DataBaseAds:Seek(f_uVar,f_oBjCol:cSortOrder,f_lSoftSeek,f_lReturnLastOrder)
   EndIf
...
Method tCtrlxBrw_EditFilterRdd(f_lClear) Class tCtrlxBrw
   ...
   For iForGets := 1 To Len(::aGetsDsFilter)
      ...
      If Len(alltrim(cTxtDigitacao)) < ::nQtaCharFilter // 4
         Loop
      EndIf
      ...
      For iForCampo := 1 To Len(aCampos)
         aadd(lc_aFieldsSeek,Upper(aCampos[iForCampo]))
         cAof += cOr +"("
         For iForTxt := 1 to Len(aTxtDigitacao)
            if len(aTxtDigitacao[iForTxt]) >= ::nQtaCharFilter
   ...
   If ::lLastRecZero
      If !Empty(::oDbDs_DataBaseAds:GetFilter())
         //? ::oDbDs_DataBaseAds:GetFilter()
         ::oDbDs_DataBaseAds:ClearFilter()
         ::oXBrwCtrl:Refresh()
         ::lGetFilterActivate    := .F.
      EndIf
      ::lGetFilterProcess := .F.
   ...
   lc_cAofAtual := ::oDbDs_DataBaseAds:GetFilter() 
   if lc_cAofAtual == cAof
      //::oXBrwCtrl:Refresh()
      //Return Nil
   EndIf
   ...
   ::lGetFilterActivate    := .t.


   If ::oDbDs_DataBaseAds:nOpenType == 1 ; // Via Rdd
          .and. !Empty(::oDbDs_DataBaseAds:cOrderFocus) // 2018-07-02  

      //wndmain():cTitle := hb_ValToExp(acampos)+"  -  "+::oDbDs_DataBaseAds:cOrderFocus
      if hb_AScan(lc_aFieldsSeek,::oDbDs_DataBaseAds:cOrderFocus) > 0 //2022-10-27
   ...
   If Len(cTxtDigitacao) < ::nQtaCharFilter-1 //3
      ::lLastRecZero := .T.
   EndIf
